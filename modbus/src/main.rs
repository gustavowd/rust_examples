...
fn read_input_reg(mut port: Box<dyn SerialPort>, endereco_slave: u8, mut init_address: u16, num_reg_read: u16, regs: &mut Vec<u16>) -> i32{
    ...
    port.write(&mensagem).expect("Write failed!");

    let mut resposta:Vec<u8> = vec![0; tamanho_resposta as usize]; //vetor  resposta

	/* Recebe a resposta */
    port.read(resposta.as_mut_slice()).expect("Found no data!");
    // endereço
    if resposta[0] != endereco_slave {
        port.clear(serialport::ClearBuffer::All).expect("Failed to clear");
		return -1;
	}
...
fn main() {
    println!("Procurando por portas seriais disponíveis");
    let ports = serialport::available_ports().expect("Nenhuma porta encontrada!");
    let mut port_name: String = "null".to_string();
    for p in ports {
        println!("{}", p.port_name);
    ...
    let name = port.name();
    if name.is_some() == true{
        println!("Porta {} aberta com sucesso!", port_name_print);
    }else{
        return;
    }
